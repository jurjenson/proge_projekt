...
def loo_set(): # loob uue seti, kui sellist nimega setti veel pole
...
def kustuta_valitud_set(): # kustutab valitud seti
...
def vali_set(): # valib seti ja laeb kaardid
    global aktiivsed_kaardid, kaardi_indeks, näitab_vastust
...
def naita_kaart(): # näitab aktiivset kaarti
...
def järgmine_kaart(): # liigub järgmisele kaardile
...
def eelmine_kaart(): # liigub eelmisele kaardile
    global kaardi_indeks
    if not aktiivsed_kaardid:
        return
...
def loe_mata_json(path="korgemmata.json"): # loeb etteantud json faili ja tagastab kaardid listina
    with open(path, "r", encoding="utf-8") as f:
        data = json.load(f)
    return [(item["küsimus"], item["vastus"]) for item in data["kaardid"]]


def hakka_oppima(): # laadib korgemmata.json kaardid ja alustab oppekaarte
    global aktiivsed_kaardid, kaardi_indeks, näitab_vastust
    try:
...
def _set_mode(text): # seab teksti kuvamise kaardi ülaosas
...
def uuenda_progress(): # uuendab progress bar'i ja labelit
...
def pööra_kaart(): # pöörab kaardi küsimuse ja vastuse vahel
    global näitab_vastust
...
def joonista_gradient(canvas, värv1=PASTEL_TOP, värv2=PASTEL_BOTTOM): # joonistab vertikaalse gradienti antud canvasile
...
if __name__ == "__main__": # peamine rakenduse tsükkel
