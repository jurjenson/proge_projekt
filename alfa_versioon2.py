...
def lae_kaardid(failinimi):
    try:
        ...
        print(f"JSON faili lugemise viga: {e}")
        return []

    if "kaardid" in andmed:
        kaardid = andmed["kaardid"]
    else:
        kaardid = andmed  # kui JSON on otse list dict'idest

    tulemus = []
    for kaart in kaardid:
        sona = kaart.get("küsimus")
        definitsioon = kaart.get("vastus")
        if sona and definitsioon:
            tulemus.append({"küsimus": sona, "vastus": definitsioon})
        else:
            print(f"Viga: kaart on puudulik ja jäetakse vahele: {kaart}")
...
if __name__ == '__main__':
    ...
    notebook.add(mata_tab, text="Kõrgem matemaatika I (alused)")

    # Siin saad lisada sisu, näiteks labelid või nupud
    ttk.Label(mata_tab, text="Tere tulemast kõrgema matemaatika alustesse!", font=('TkDefaultFont', 18)).pack(padx=10, pady=20)

    # Näide nuppudest või interaktiivsetest elementidest
    ttk.Button(mata_tab, text="Hakkame õppima").pack(padx=10, pady=10)

    def tab_vahetus(event):
        valitud_tab = event.widget.tab(event.widget.index("current"))["text"]
        if valitud_tab == "Kõrgem matemaatika I (alused)":
            failinimi = 'kõrgemmata.json'
            seti_nimi = 'Kõrgem matemaatika I (alused)'

            # Lae JSON ainult üks kord
            if getattr(tab_vahetus, "laetud", False):
                return

            lae_kaardid(failinimi)

            # Nüüd, kui GUI on valmis, vali set ja kuva kaardid
            seti_valik.set(seti_nimi)
            vali_set()

            tab_vahetus.laetud = True
            
    notebook.bind("<<NotebookTabChanged>>", tab_vahetus)
